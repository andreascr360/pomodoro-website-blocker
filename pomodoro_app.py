...
DEFAULT_EATING_BREAK_DURATION_MINUTES = 30 # NEW
...
# CIRCLE_FG_COLOR_EATING_BREAK = "sky blue" # Optional distinct color
...
# Stop Icon Constants (NEW)
STOP_ICON_SIZE = 30 # Size of the clickable canvas for the stop icon
STOP_ICON_PADDING = 5 # Padding inside the canvas to draw the square
STOP_ICON_SQUARE_COLOR_ACTIVE = "firebrick" # Color of the stop square when active
STOP_ICON_SQUARE_COLOR_DISABLED = "gray60" # Color when disabled

PAUSE_PLAY_ICON_SIZE = 30 # Same size as stop icon for consistency
PAUSE_PLAY_ICON_PADDING = 5
PAUSE_ICON_COLOR_ACTIVE = "dodger blue" # Or any color you like
...
class PomodoroWebsiteBlocker:
    def __init__(self, root_window):
        ...
        self.root.geometry(f"500x{580 + CIRCLE_CANVAS_SIZE // 3}")
        ...
        self.eating_break_duration_minutes = DEFAULT_EATING_BREAK_DURATION_MINUTES # NEW
        self.pomodoros_for_full_xp = DEFAULT_POMODOROS_FOR_FULL_XP
        self._load_settings() # Loads all settings including new eating_break_duration_minutes

        self.blocked_websites = set()
        self.pomodoro_count = 0
        self.timer_running = False
        self.current_state = "Idle" # "Idle", "Focus", "Break" (Break covers short, long, eating)
        self.current_break_type = "" # NEW: To store "Short", "Long", "Eating" for specific label updates
        ...
        # --- Row 2 ---
        self.timer_canvas = tk.Canvas(self.root, width=CIRCLE_CANVAS_SIZE, height=CIRCLE_CANVAS_SIZE,
                                      bg=self.root.cget('bg'), highlightthickness=0)
        self.timer_canvas.grid(row=2, column=0, columnspan=4, padx=10, pady=10)
        self._setup_timer_canvas_elements()
        
        self.stop_icon_canvas = tk.Canvas(self.root, width=STOP_ICON_SIZE, height=STOP_ICON_SIZE,
                                          bg=self.root.cget('bg'), highlightthickness=0, cursor="hand2")
        self.stop_icon_canvas.grid(row=2, column=3, padx=(0,20), pady=10, sticky="e") # In the 4th column
        ...
        self.start_focus_button.grid(row=3, column=0, columnspan=2, padx=5, pady=10, sticky="ew") # Spans 3 columns
        
        self.start_eating_break_button = ttk.Button(self.root, text="", command=self._start_eating_break_session)
        self.start_eating_break_button.grid(row=3, column=2, columnspan=2, padx=5, pady=10, sticky="ew") # Spans all 4 columns
    ...
    def _draw_xp_bar(self):
        ...
        if canvas_width <= 1: canvas_width = int(500 * XP_BAR_WIDTH_FACTOR * 0.95)
        bar_width = canvas_width - 20; bar_x_start = 10; bar_y_start = 5
        ...
        self.xp_bar_canvas.create_text(bar_x_start + bar_width / 2, bar_y_start + XP_BAR_HEIGHT + 10, text=xp_text, font=("Helvetica", 10), fill=XP_BAR_TEXT_COLOR, anchor=tk.CENTER) # Adjusted y for text
    ...
    def _update_timer_display(self): # UPDATED for specific break labels
        mins, secs = divmod(self.remaining_seconds, 60)
        time_format = f"{mins:02d}:{secs:02d}"
        self.timer_canvas.itemconfig(self.time_text_id, text=time_format)
        progress_extent = 0
        current_fg_color = CIRCLE_FG_COLOR_FOCUS # Default
        ...
        if self.timer_running and self.total_seconds_for_session > 0:
            ...
            if self.current_state == "Focus":
                ...
                current_fg_color = CIRCLE_FG_COLOR_BREAK # General break color
                # Example: if self.current_break_type == "Eating": current_fg_color = CIRCLE_FG_COLOR_EATING_BREAK
        
        self.timer_canvas.itemconfig(self.progress_arc_id, extent=-progress_extent, outline=current_fg_color)
        
        # Update status label based on precise state
        if self.timer_running:
            if self.current_state == "Focus":
                self.timer_label.config(text="Status: Focus Time")
            elif self.current_state == "Break":
                if self.current_break_type == "Short":
                    self.timer_label.config(text=f"Status: Short Break ({self.short_break_duration_minutes} min)")
                elif self.current_break_type == "Long":
                    self.timer_label.config(text=f"Status: Long Break ({self.long_break_duration_minutes} min)")
                elif self.current_break_type == "Eating":
                    self.timer_label.config(text=f"Status: Eating Break ({self.eating_break_duration_minutes} min)")
    ...
    def _create_menubar(self): # UPDATED
        ...
        edit_menu.add_command(label="Set Eating Break Duration...", command=self._edit_eating_break_duration) # NEW
        edit_menu.add_separator()
        edit_menu.add_command(label="Set Pomodoros for Full XP...", command=self._edit_pomodoros_for_full_xp)
        menubar.add_cascade(label="Edit", menu=edit_menu)
        self.root.config(menu=menubar)

    def _load_settings(self): # UPDATED
        try:
            if CONFIG_FILE_PATH.exists():
                with open(CONFIG_FILE_PATH, "r", encoding='utf-8') as f:
                    ...
                    self.eating_break_duration_minutes = int(settings.get("eating_break_duration_minutes", DEFAULT_EATING_BREAK_DURATION_MINUTES)) # NEW
                    ...
                    if not (5 <= self.eating_break_duration_minutes <= 120): self.eating_break_duration_minutes = DEFAULT_EATING_BREAK_DURATION_MINUTES # NEW validation
            ...
            print(f"Error loading settings: {e}. Using defaults."); self._reset_to_default_settings_and_save()
        except Exception as e:
            print(f"Unexpected error loading settings: {e}. Using defaults."); self._reset_to_default_settings_and_save()

    def _reset_to_default_settings_and_save(self): # UPDATED
        self.focus_duration_minutes = DEFAULT_FOCUS_DURATION_MINUTES
        self.short_break_duration_minutes = DEFAULT_SHORT_BREAK_DURATION_MINUTES
        self.long_break_duration_minutes = DEFAULT_LONG_BREAK_DURATION_MINUTES
        self.eating_break_duration_minutes = DEFAULT_EATING_BREAK_DURATION_MINUTES # NEW
    ...
    def _save_settings(self): # UPDATED
        settings = {
            ...
            "eating_break_duration_minutes": self.eating_break_duration_minutes, # NEW
    ...
    def _edit_eating_break_duration(self): # NEW
        new_duration = simpledialog.askinteger("Eating Break Duration", 
                                            "Enter eating break duration (minutes, 5-120):",
                                            parent=self.root, minvalue=5, maxvalue=120,
                                            initialvalue=self.eating_break_duration_minutes)
        if new_duration is not None:
            self.eating_break_duration_minutes = new_duration
            self._update_button_labels()
            self._save_settings()
    ...
    def _edit_pomodoros_for_full_xp(self):
        ...
        if new_value is not None:
            self.pomodoros_for_full_xp = new_value
            self._save_settings(); self._draw_xp_bar()
    ...
    def _update_button_labels(self): # UPDATED
        ...
        self.start_eating_break_button.config(text=f"Eating Break ({self.eating_break_duration_minutes} min)") # NEW

    def _update_ui_for_timer_state(self): # UPDATED
        timer_is_active = self.timer_running
        self.start_focus_button.config(state=tk.DISABLED if timer_is_active else tk.NORMAL)
        self.start_short_break_button.config(state=tk.DISABLED if timer_is_active else tk.NORMAL)
        self.start_long_break_button.config(state=tk.DISABLED if timer_is_active else tk.NORMAL)
        self.start_eating_break_button.config(state=tk.DISABLED if timer_is_active else tk.NORMAL) # NEW
        self._draw_stop_icon(is_enabled=timer_is_active) # Update stop icon state
        self._update_timer_display() # Always update timer display

    def _handle_natural_session_completion(self): # UPDATED (no auto break start)
        ...
        if session_that_completed == "Focus":
            ...
            self._reset_session_end_actions() # Go to Idle
    ...
    def _draw_stop_icon(self, is_enabled=True): # NEW
        ...
        # Draw the square stop symbol
        self.stop_icon_canvas.create_rectangle(
            STOP_ICON_PADDING, STOP_ICON_PADDING, 
            STOP_ICON_SIZE - STOP_ICON_PADDING, STOP_ICON_SIZE - STOP_ICON_PADDING,
            fill=square_color, outline=square_color
        )

    def _on_stop_icon_click(self, event=None): # NEW
        if self.timer_running: # Only if timer is running (icon should be enabled)
            self._stop_current_session()

    def _start_focus_session(self):
        if self.timer_running: messagebox.showwarning("Timer Active", "A session is already in progress.", parent=self.root); return
        if not self.blocked_websites and not messagebox.askyesno("No Websites Blocked", "Your block list is empty. Start focus anyway?", parent=self.root): return
        
        self.timer_running = True
        self.current_state = "Focus"
        self.current_break_type = "" # Not a break
        self.total_seconds_for_session = self.focus_duration_minutes * 60
        self.remaining_seconds = self.total_seconds_for_session
        
        self._update_ui_for_timer_state()
        if self.blocked_websites: self._block_domains(list(self.blocked_websites))
    ...
    def _start_short_break_session(self):
        ...
        self.current_break_type = "Short" # Set break type
        self.total_seconds_for_session = self.short_break_duration_minutes * 60
        self.remaining_seconds = self.total_seconds_for_session
        self._update_ui_for_timer_state()
        self._tick_countdown()
    ...
    def _start_long_break_session(self):
        ...
        self.timer_running = True
        self.current_state = "Break"
        self.current_break_type = "Long" # Set break type
        self.total_seconds_for_session = self.long_break_duration_minutes * 60
        self.remaining_seconds = self.total_seconds_for_session
        self._update_ui_for_timer_state()
        self._tick_countdown()

    def _start_eating_break_session(self): # NEW
        if self.timer_running: messagebox.showwarning("Timer Active", "A session is already in progress.", parent=self.root); return
        self.timer_running = True
        self.current_state = "Break"
        self.current_break_type = "Eating" # Set break type
        self.total_seconds_for_session = self.eating_break_duration_minutes * 60
        self.remaining_seconds = self.total_seconds_for_session
        self._update_ui_for_timer_state()
        self._tick_countdown()

    def _stop_current_session(self): # Renamed original to this, used by stop icon
        if not self.timer_running: return
        was_focus_session = (self.current_state == "Focus")
        self.timer_running = False
        if self._timer_id: self.root.after_cancel(self._timer_id); self._timer_id = None
        
        # Store break type before resetting state for message purposes
        ...
        if was_focus_session and self.blocked_websites:
            ...
            print("Focus session stopped by user.") # Or update timer_label
        elif stopped_break_type:
            print(f"{stopped_break_type} break stopped by user.")
    ...
    def _reset_session_end_actions(self):
        ...
        if was_focus_before_idle and self.blocked_websites:
            print("Failsafe unblock in _reset_session_end_actions for a focus session transitioning to idle.")
            self._unblock_domains(list(self.blocked_websites))
        self._update_ui_for_timer_state()
    ...
    def on_closing(self):
        self._save_settings()
        if self.timer_running:
            if self.current_state == "Focus" and self.blocked_websites:
                print("Application closing during focus: Unblocking sites.")
                self._unblock_domains(list(self.blocked_websites))
            self.timer_running = False
            if self._timer_id: self.root.after_cancel(self._timer_id); self._timer_id = None
    ...
    # --- Methods that remain unchanged from previous full version ---
    def _open_block_list_manager(self):
        if self.block_list_manager_window is None or not self.block_list_manager_window.winfo_exists():
            ...
            self.block_list_manager_window.lift(); self.block_list_manager_window.focus_force()
    ...
    def _block_domains(self, domains_to_block_list):
        ...
        for line in original_lines:
            ...
            else: modified = True # Line removed
        new_entries = [f"{REDIRECT_IP}\t{md}\t{POMODORO_COMMENT}\n" for md in sorted(list(variants_to_block))]
        if modified or new_entries: # If lines were removed or new ones to add
